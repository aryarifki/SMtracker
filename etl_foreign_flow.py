...
def upsert_foreign_analytics(ticker: str, lookback_days: int, payload: dict) -> None:
    ...
    try:
        with raw_conn.cursor() as cur:
            # Format JSONB membutuhkan string JSON
            row = (
                ticker,
                lookback_days,
                payload["latest_date"],
...
def process_ticker(ticker: str, lookback_days: int):
    ...
    # 1. Ambil tanggal terbaru dari broker_flow
    ...
    start_date = end_date - timedelta(days=lookback_days * 2) # Buffer untuk hari libur
    
    # 2. Ambil Macro Data (Harga & Foreign Flow)
    ...
    # Forward-fill harga close
    closes = []
    last_close = 0.0
    # Ambil harga dari tabel prices
    ...
    # 3. Ambil Micro Data (Broker Activity per hari)
